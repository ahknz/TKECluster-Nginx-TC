...
resource "tencentcloud_cam_policy" "TkeAsMgr" {
  count = var.create_cam_strategy ? 1 : 0
  name = "TF_QcloudAccessForASRole"
  document = jsonencode({
    "version": "2.0",
    "statement": [
        {
...
resource "tencentcloud_cam_policy" "TkeCfsMgr" {
  count = var.create_cam_strategy ? 1 : 0
  name = "TF_QcloudAccessForTKERoleInCreatingCFSStorageclass"
  document = jsonencode({
    "version": "2.0",
    "statement": [
        {
...
resource "tencentcloud_cam_role_policy_attachment" "QCS_CFS" {
  count = var.create_cam_strategy ? 1 : 0

  role_id   = lookup(tencentcloud_cam_role.TKE_QSCRole.0, "id")
  policy_id = lookup(tencentcloud_cam_policy.TkeCfsMgr.0, "id")
}

resource "tencentcloud_cam_role_policy_attachment" "QCS_AS" {
  count = var.create_cam_strategy ? 1 : 0

  role_id   = lookup(tencentcloud_cam_role.TKE_QSCRole.0, "id")
  policy_id = lookup(tencentcloud_cam_policy.TkeAsMgr.0, "id")
}
